...
datapath = os.path.join("data", "abstract2keyword.tfrecord")
...
def feature_label_pairs(example):
  parsed_example = tf.io.parse_single_example(
      example, {
          ...
          'keyword': tf.io.FixedLenFeature(shape=(title_maxlen), dtype=tf.int64),
          # I made a typo and now it's baked in lmao
          'titlte_out': tf.io.FixedLenFeature(shape=(title_maxlen), dtype=tf.int64)
      })
  return {"inputs": parsed_example['abstract'], "dec_inputs": parsed_example['keyword']}, parsed_example['titlte_out']
...
path_checkpoint = 'abstract2keywords_transformer_checkpoint.keras'

path_checkpoint_backup = 'abstract2keywords_transformer_checkpoint_backup.keras'
